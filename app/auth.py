...
from .routes import load_json  # reuse your JSON loader
...
def login():
    ...
    if request.method == 'POST':
        ...
        if not user or not user.check_password(password):
            ...
            return render_template('auth/login.html', site=site)
...
def register():
    ...
    if request.method == 'POST':
        ...
        if not email or not password:
            ...
            return render_template('auth/register.html', site=site)
...
@login_required
def logout():
    site = load_json('site.json')  # not strictly needed, but for consistency
    logout_user()
