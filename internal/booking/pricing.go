...
// Prices contains the flat per-booking prices used by the Flask API.
type Prices struct {
	FreeSwim     int64 `json:"free_swim"`
	LaneTraining int64 `json:"lane_training"`
}

// DefaultPrices returns the Flask route's fallback booking prices.
...
// LoadPrices loads prices.json and falls back exactly as the Flask route does
